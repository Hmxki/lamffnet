...
from src.add_encoder import AMSUnet as addencoder
from src.add_skip_connection import AMSUnet as wff
...
def create_model(num_classes):
    #model = wff(in_channels=3, num_classes=num_classes, base_c=32)
    # model = addencoder(in_channels=3, num_classes=num_classes, base_c=32)
    model = msscnet(in_channels=3, num_classes=num_classes,base_c=32)
    # model = UNet(in_channels=3, num_classes=num_classes)
    #model = MALUNet(input_channels=3, num_classes=num_classes)
...
def main(args):
    ...
    # std = (0.224, 0.244, 0.244)




    # 用来保存训练以及验证过程中信息
    results_file = "./results/results{}cvc{}_KD.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"),args.count)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.save_best is True:
            torch.save(save_file, "save_weights/cvc{}_KD.pth".format(args.count))
        else:
...
def parse_args():
    ...
    parser.add_argument("--data-path", default="./cvc", help="DRIVE root")
    # exclude background
    ...
    parser.add_argument('--count', default=3, type=int, help='')
